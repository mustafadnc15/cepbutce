...
**Working directory:** All task paths are relative to `/Users/mustafadinc/studio-apps/budget-app/`, which is also the React Native project root.

**Note on layout:** This plan was written assuming the scaffold lived at `budget-app/CepButce/`. After Phase 0, the project was flattened to repo root. Path references below have been updated to drop the `CepButce/` prefix; commits from before the flatten still reference the nested path.
...
- `package.json`, `tsconfig.json`, `App.tsx`, `index.js`
- `babel.config.js` — we add Reanimated plugin + NativeWind preset
- `metro.config.js` — we wrap with `withNativeWind`
- `ios/Podfile.properties.json`, `android/gradle.properties` — verify New Arch
- `android/app/build.gradle` — we append vector-icons fonts.gradle
- `.gitignore` — left as-is, committed from scaffold

**Created by us:**
- `global.css` — NativeWind tailwind directives
- `tailwind.config.js` — mirrors theme tokens
- `nativewind-env.d.ts` — NativeWind type augmentation
- `react-native.config.js` — Feather font asset linking
- `src/theme/tokens.js` — CommonJS token source-of-truth
- `src/theme/colors.ts`, `typography.ts`, `spacing.ts`, `shadows.ts`, `radius.ts`, `index.ts`
- `src/theme/ThemeProvider.tsx` — Context + `useTheme` hook
- `src/components/ui/{Card,StatCard,IconBox,ListCard,Input,Button,SectionHeader,Chip,EmptyState}.tsx`
- `src/components/ui/index.ts` — barrel
- `src/db/connection.ts`, `src/db/migrations.ts`
- `src/stores/mmkv.ts` — MMKV Zustand adapter
- `src/stores/useSettingsStore.ts`
- `src/stores/useTransactionStore.ts`
- `src/stores/useSubscriptionStore.ts`
- `src/stores/useBudgetStore.ts`
- `src/i18n/index.ts`, `tr.json`, `en.json`
- `src/utils/currency.ts`, `date.ts`, `id.ts`
- `src/types/index.ts`
- `src/constants/categories.ts`, `currencies.ts`
- `__tests__/utils/currency.test.ts`, `date.test.ts`, `id.test.ts`
- `.keep` files for deferred dirs: `src/components/{navigation,dashboard,transactions,subscriptions,scanner}/.keep`, `src/screens/.keep`, `src/navigation/.keep`, `src/services/.keep`
...
Goal: the project compiles on both platforms with every listed native dep installed and no autolink warnings.
...
**Files:**
- Create: scaffolded RN project at the repo root (entire tree)
...
Expected: scaffold completes, Metro does NOT auto-start, the `CepButce/` directory is populated with `ios/`, `android/`, `App.tsx`, `package.json`, `tsconfig.json`, `Gemfile`, `.gitignore`. `npm install` runs automatically. `pod install` may or may not auto-run depending on CLI version. (Files were later moved up to the repo root in a post-Phase-0 restructure; the `init` step itself still creates the nested `CepButce/` dir.)
...
Expected: `ls: .git: No such file or directory`. If a `.git` exists, delete it: `rm -rf .git`.
...
**Files:**
- Modify (conditionally): `ios/Podfile.properties.json`
- Modify (conditionally): `android/gradle.properties`
...
```bash
grep -E '^newArchEnabled' android/gradle.properties
```
...
```bash
cd /Users/mustafadinc/studio-apps/budget-app
git status ios/Podfile.properties.json android/gradle.properties
# If either shows modified:
git add ios/Podfile.properties.json android/gradle.properties
...
```bash
cd /Users/mustafadinc/studio-apps/budget-app
git add metro.config.js global.css nativewind-env.d.ts
...
```bash
ls /Users/mustafadinc/studio-apps/budget-app/node_modules/@react-native-vector-icons/feather/fonts/
```
...
```bash
cat /Users/mustafadinc/studio-apps/budget-app/node_modules/@react-native-vector-icons/feather/react-native.config.js
```
...
```bash
cd /Users/mustafadinc/studio-apps/budget-app/ios
bundle install
```
...
> Checkpoint 1 complete. Native scaffold is in place at the project root. All JS packages installed, `pod install` clean, Metro starts cleanly. Please run `npx react-native run-ios` and `npx react-native run-android` in separate terminals, confirm the stock RN welcome screen appears on both, and let me know when to proceed to Checkpoint 2.
...
```bash
ls src/stores/mmkv.ts src/stores/useSettingsStore.ts
```
...
Append these lines to the end of `src/theme/index.ts`:
...
- Create: `__tests__/utils/id.test.ts`
...
File: `__tests__/utils/id.test.ts`
...
File: `__tests__/utils/date.test.ts`
...
File: `src/stores/useTransactionStore.ts`
...
```bash
cd /Users/mustafadinc/studio-apps/budget-app
git add src/stores/useTransactionStore.ts
...
File: `src/stores/useSubscriptionStore.ts`
...
```bash
cd /Users/mustafadinc/studio-apps/budget-app
git add src/stores/useSubscriptionStore.ts
...
```bash
cd /Users/mustafadinc/studio-apps/budget-app
git add src/stores/useBudgetStore.ts
...
File: `src/components/ui/IconBox.tsx`
...
File: `src/components/ui/ListCard.tsx`
...
```bash
cd /Users/mustafadinc/studio-apps/budget-app
git add src/components/ui/IconBox.tsx src/components/ui/StatCard.tsx src/components/ui/ListCard.tsx
...
**Files:**
- Create: `src/components/navigation/.keep`
- Create: `src/components/dashboard/.keep`
- Create: `src/components/transactions/.keep`
- Create: `src/components/subscriptions/.keep`
- Create: `src/components/scanner/.keep`
- Create: `src/screens/.keep`
- Create: `src/navigation/.keep`
- Create: `src/services/.keep`
...
```bash
cd /Users/mustafadinc/studio-apps/budget-app
git add src/components/navigation src/components/dashboard src/components/transactions src/components/subscriptions src/components/scanner src/screens src/navigation src/services
